...
slot_coins = Template(r"tpl1689318326388.png", record_pos=(-0.407, -0.906), resolution=(1080, 2316))
...
def adb_screenshot():
        '''截图'''
        time_name = datetime.now().strftime('%m%d_%H%M%S')
        Pictures_name = f'{time_name}.png'
        shell(f"screencap -p /sdcard/{Pictures_name}")
        sleep(3)
        print(f"{Pictures_name}截图已经保存")
