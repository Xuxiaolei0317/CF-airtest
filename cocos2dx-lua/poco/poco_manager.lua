...
local VERSION = import('poco.POCO_SDK_VERSION')
local Dumper = import('poco.Cocos2dxFrozenDumper')
local Screen = import('poco.Cocos2dxScreen')
local ClientConnection = import('poco.ClientConnection')
local json = import('poco.support.dkjson')
...
local function isRunLuaAllowed()
    return (bole and bole.isPocoPackage) or appDebugMode == true
end

local function normalizeRunLuaResult(value, visited)
    ...
    for k, v in pairs(value) do
        ...
        result[safeKey] = normalizeRunLuaResult(v, visited)
    end
    visited[value] = nil
...
local function packRunLuaResults(...)
    return {n = select('#', ...), ...}
end
...
local dispatcher = {
    ...
    RunLua = function(code)
        if not isRunLuaAllowed() then
            return {success = false, error = 'RunLua only allowed in poco/debug package'}
        end

        if type(code) ~= 'string' or code == '' then
            return {success = false, error = 'RunLua requires non-empty lua code'}
        end
        ...
        local ok, err = xpcall(function()
            returned = packRunLuaResults(func())
        end, debug.traceback)
        if not ok then
            return {success = false, error = tostring(err)}
        end
        ...
        if returned.n == 0 then
            return {success = true, hasResult = false, result = json.null}
        end

        if returned.n == 1 then
            return {success = true, hasResult = true, result = normalizeRunLuaResult(returned[1])}
        end

        local results = {}
        for i = 1, returned.n do
            results[i] = normalizeRunLuaResult(returned[i])
        end
        return {success = true, hasResult = true, result = results}
    end,
